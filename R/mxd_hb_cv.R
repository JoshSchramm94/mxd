...
#'
#' @details
#' `mxd_hb_cv()` runs the k-fold hierarchical Bayes cross-validation for MaxDiff data.
#' `data_stan` needs to be a list with named parameters. This is provided using
#' the `dm_to_stan_hb_cv()` in the `stan_input` list. `chains` defines the number
...
#' }
#'
#' `mxd_hb_cv()` uses the \code{\link[rstan]{stan}} for sampling, thus, a more
